...
### 碰撞相关函数
1)添加碰撞盒子并实例化，这里需要用到.AddDynamic()函数
```
//.h文件
public:	
//添加碰撞盒子
...
虚函数定义使，变量前面有class，如virtual void OnHitSphere(class ASphereBase * sphere);
```
//.h文件
//声明自定义OnCollision函数
UFUNCTION()
...
```
UFUNCTION(BlueprintCallable)
	void SpeedControl(bool val);
```
2).cpp文件中定义
```
void ASphereBase::SpeedControl(bool val)
{
	if (val && AngularVelocity != FVector(0, 0, 0))
	{
...
1).h文件中创建一个坐标变量sphereVEC;关于碰撞函数和碰撞事件卸载HitBoxBase文件中，从HitBoxBase类中继承并覆写OnHitSphere函数
```
public:
UPROPERTY(EditAnywhere, BlueprintReadWrite,Category = "DieSphere")
	FVector sphereVEC;
public:
...
```
2).cpp文件中定义覆写OnHitSphere函数
```
void ADieSphere::OnHitSphere(ASphereBase * sphere)
{
	sphere->SetActorLocation(sphereVEC);
}
```
## .和::和：和->的区别
